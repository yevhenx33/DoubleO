...
class OrthogonalDoubleOBlock(nn.Module):
    def __init__(self, dim, expansion, iterations, dropout=0.1):
        super().__init__()
        self.iterations = iterations
        self.dropout = nn.Dropout(dropout)
        
        # Skew-symmetric parameter for Cayley Transform
        self.W_skew = nn.Parameter(torch.randn(dim, dim) * (1.0 / dim))
    ...
    def forward(self, x):
        ...
        B, T, D = x.size()
        pad_len = (4 - T % 4) % 4
        if pad_len > 0: x_scan = F.pad(x, (0, 0, 0, pad_len))
        else: x_scan = x
        
        # Cayley Transform: A = (I - S) @ (I + S)^-1
        # Ensures A is strictly orthogonal (eigenvalues on the unit circle)
        S = self.W_skew - self.W_skew.transpose(0, 1)
        I = torch.eye(D, device=x.device)
        A = torch.linalg.solve(I + S, I - S) 
        ...
        # Cavity Iterations with Dropout
        for _ in range(self.iterations):
            h_hidden = F.gelu(h_norm @ self.W_in + self.b_in)
            h_hidden = self.dropout(h_hidden)
            h_delta = h_hidden @ self.W_out + self.b_out
            h_norm = h_norm + h_delta
            
        return x + self.dropout(h_norm)

class OrthogonalDoubleO(nn.Module):
    def __init__(self, vocab_size, config):
        super().__init__()
        dim = 128
        self.token_emb = nn.Embedding(vocab_size, dim)
        self.blocks = nn.ModuleList([
            OrthogonalDoubleOBlock(
                dim=dim,
...
def run_model_eval(model_name: str):
    ...
    config = {"num_layers": 6, "expansion": 4, "iterations": 4, "dropout": 0.1}
    model = OrthogonalDoubleO(vocab_size=vocab_size, config=config).cuda()
...
def main():
    models = ["orthogonal_pure"]
    
    print("Launching Orthogonal Regularization 5k-step Showdown on Modal...")
    ...
    with open("data/orthogonal_5k_results.json", "w") as f:
        json.dump(final_dict, f)
        
    print("Orthogonal 5k Showdown complete! Results saved to data/orthogonal_5k_results.json")
