...
class RadicalDoubleOBlock(nn.Module):
    def __init__(self, dim, expansion, iterations):
        super().__init__()
        self.iterations = iterations
        
        # Pre-Scan Stabilization
        self.norm_scan = nn.RMSNorm(dim)
        
        # Multi-Band Vector Decay
        ...
        self.W_gate = nn.Parameter(torch.randn(dim, inner_dim) * (1.0 / dim))
        self.W_up = nn.Parameter(torch.randn(dim, inner_dim) * (1.0 / dim))
        self.W_down = nn.Parameter(torch.randn(inner_dim, dim) * (1.0 / dim))
        
        self.b_gate = nn.Parameter(torch.zeros(inner_dim))
        self.b_up = nn.Parameter(torch.zeros(inner_dim))
    ...
    def forward(self, x):
        ...
        # Pre-Scan Norm
        x_scan = self.norm_scan(x)
        if pad_len > 0: x_scan = F.pad(x_scan, (0, 0, 0, pad_len))
        
        # Vector Decay
        A = self.W_a * torch.sigmoid(self.decay).unsqueeze(0)
        ...
        for _ in range(self.iterations):
            gate = F.silu(h_norm @ self.W_gate + self.b_gate)
            up = h_norm @ self.W_up + self.b_up
            h_hidden = gate * up
            h_delta = h_hidden @ self.W_down + self.b_down
            h_norm = h_norm + h_delta
            
        return x + h_norm 

class RadicalDoubleO(nn.Module):
...
def run_model_eval(model_name: str):
    ...
    config = {"num_layers": 8, "expansion": 2, "iterations": 4}
    model = RadicalDoubleO(vocab_size=vocab_size, config=config).cuda()
...
def main():
    ...
    print("Launching Radical Optimization 5k-step Gauntlet on Modal...")
    ...
    with open("data/radical_5k_results.json", "w") as f:
        json.dump(final_dict, f)
        
    print("Radical 5k Showdown complete! Results saved to data/radical_5k_results.json")
